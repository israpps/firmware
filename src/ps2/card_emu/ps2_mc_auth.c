...
uint8_t arcade_key[16]  = {0xA8, 0x2C, 0x4C, 0xF3, 0x8F, 0x63, 0x82, 0x76, 0x53, 0x0E, 0xE1, 0x57, 0xDA, 0xCF, 0xBB, 0x67}; // SCPH-10020 in Arcade mode
...
void __time_critical_func(generateIvSeedNonce)() {
    ...
    if (settings_get_ps2_variant() == PS2_VARIANT_SC2) {
        uint8_t conq_iv[8]    = {0x01, 0x11, 0xA2, 0x00, 0x14, 0x02, 0x02, 0x61};
        uint8_t conq_seed[8]  = {0x01, 0x01, 0x52, 0x44, 0x01, 0x90, 0xF5, 0x73};
        uint8_t conq_nonce[8] = {0xD6, 0xFF, 0x30, 0x36, 0x52, 0xEA, 0xFE, 0x5E};
        for (int i = 0; i < 8; i++) {
            iv[i] = conq_iv[i];
            seed[i] = conq_seed[i];
            nonce[i] = conq_nonce[i];
        }
    } else {
        for (int i = 0; i < 8; i++) {
            iv[i] = 0x42;
